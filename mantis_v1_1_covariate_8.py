# -*- coding: utf-8 -*-
"""mantis_v1.1_covariate_8.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1u8Io0v-N0WFMqUfcq204kIuUWdALJE9I
"""
...
def quantile_loss(preds, target, quantiles=[0.05, 0.1, 0.25, 0.4, 0.5, 0.6, 0.75, 0.9, 0.95]):
    """
...
def evaluate_on_us_states(
    ...
    for state in state_cols:
        ...
        for start in range(min_context, max_start):
            ...
            # Extract quantiles (assuming 9 quantiles: 0.05, 0.1, 0.25, 0.4, 0.5, 0.6, 0.75, 0.9, 0.95)
            med_scaled, q05_scaled, q95_scaled = q_preds[:, 4], q_preds[:, 0], q_preds[:, 8]
        ...
        if seq_mae:
            ...
            print(f"{state}: MAE={mae:.4f}, 90%-cov={cover*100:.1f}%")

    overall_mae = np.mean(all_mae) if all_mae else float('nan')
    overall_cov = np.mean(all_cov) if all_cov else float('nan')

    print(f"\nUS-state {type_to_name[target_type].upper()} forecast from {type_to_name[covariate_type]} – overall MAE: {overall_mae:.4f}, 90% coverage: {overall_cov*100:.1f}%")
...
def evaluate_on_mi_regions(
    ...
    for region in region_cols:
        ...
        for start_idx in range(min_context, max_idx):
            ...
            pred_median_scaled = pred_all_quantiles[:, 4]
            pred_q05_scaled = pred_all_quantiles[:, 0]
            pred_q95_scaled = pred_all_quantiles[:, 8]
        ...
        print(f"{region}: MAE = {mae:.4f}, 90% Coverage = {coverage * 100:.2f}%")
            ...
            for batch_idx, batch in enumerate(pbar):
                ...
                if (steps_done % 5000) == 0 and steps_done > 0:
                    ...
                    if avg_loss < best_loss:
                        best_loss = avg_loss
                        save_path = os.path.join(save_dir, f"best_model_{timestamp}_8weekcovariate.pt")
                    else:
                        save_path = os.path.join(save_dir, f"forecaster_checkpoint_{timestamp}_8weekcovariate.pt")
                ...
                if (steps_done % eval_interval) == 0:
                    ...
                    try:
                        ...
                        if eval_data_dir:
                            ...
                            print(f"\nUS-states overall MAE: {results['overall_mae']:.4f}  90% coverage: {results['overall_coverage']*100:.2f}%")
...
if __name__ == "__main__":
    ...
    ckpt_paths = glob.glob(os.path.join(model_dir, "*_8weekcovariate.pt"))
    ...
    train_losses = train_multi_time_series_forecaster(
        ...
        save_dir=model_dir,
        gradient_clip_val=1.0,
        initial_teacher_forcing_ratio=0.2,
